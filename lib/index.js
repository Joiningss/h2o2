...
internals.handler = function (route, handlerOptions) {
    ...
    Hoek.assert(!route.settings.payload || (route.settings.payload.output === 'data') || ((route.settings.payload.output === 'stream') && !route.settings.payload.parse), 'Cannot proxy if (payload is parsed and output is stream) or if output is not stream or data');
    ...
    return function (request, reply) {
        ...
        if(request.payload){
            if(request.route.settings.payload.output === 'data' && request.route.settings.payload.parse){
